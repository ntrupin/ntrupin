import markdown
from flask import (
    Blueprint, render_template, g, abort, redirect, url_for, flash,
    request, abort
)
...
def show_id(id):
    return show(get_page_by_id(id))

@bp.route("/<string:name>", methods=["GET"])
def show_name(name):
    return show(get_page_by_fuzzy_name(name))
...
@login_required
def delete(id):
    query("DELETE FROM pages WHERE id = %s", args=[id])
    return redirect(url_for("pages.index"))
